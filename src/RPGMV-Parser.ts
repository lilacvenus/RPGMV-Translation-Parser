...
function replacePatterns(input: string): string[] {
    const step1 = input.split(/(\\C\[\d+\])/);             // Split on "\\C[21]"
    const step2 = step1.flatMap(item => item.split(/(\\fb)/));                   // Split on "\\fb"
    const step3 = step2.flatMap(item => item.split(/(\\msgposy\[\d+\])/));       // Split on "\\msgposy[455]"
    const step4 = step3.flatMap(item => item.split(/(\\msgwidth\[[^\]]+\])/));   // Split on "\\msgwidth[auto]"
    const step5 = step4.flatMap(item => item.split(/(\\lsoff)/));                // Split on "\\lsoff"
    const step6 = step5.flatMap(item => item.split(/(\\>)/));                    // Split on "\\>"
    const step7 = step6.flatMap(item => item.split(/(\\m\[[^\]]+\])/));          // Split on "\\m[Cerio]"
    const step8 = step7.flatMap(item => item.split(/(\\autoevent\[\d+\])/));     // Split on "\\autoevent[1]"
    ...
    const step11 = step10.flatMap(item => item.split(/(\\TA\[\d+\])/));          // Split on "\\TA[1]"
    const step12 = step11.flatMap(item => item.split(/(\\lson)/));               // Split on "\\lson"
    const step13 = step12.flatMap(item => item.split(/(\\\|)/));                 // Split on "\\|"
    const step14 = step13.flatMap(item => item.split(/(\\\^)/));                 // Split on "\\^"
    const step15 = step14.flatMap(item => item.split(/(\\\.\n)/));               // Split on "\\.\n"
